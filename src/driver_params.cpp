...
tape_sort::DriverParams::DriverParams(const ConfigReader& config)
    : max_elem_number(config.get<size_t>("driver_settings", "max_elem_number")),
      ...
      temp_file_dir_name(
          config.get<std::string>("driver_settings", "temp_file_dir_name")),
      temp_file_format(
          config.get<std::string>("driver_settings", "temp_file_format")){};

tape_sort::DriverParams::DriverParams(
    size_t max_elem_number, uint32_t item_write_delay, uint32_t item_read_delay,
    uint32_t tape_rewind_delay, uint32_t step_shift_delay,
    std::string temp_file_dir_name, std::string temp_file_format)
