...
public:
    static std::vector<int32_t> ReadTape(std::string& tape_name)
    {
        ...
        while (input_tape >> curr)
            input_data.push_back(curr);
        input_tape.close();
        return input_data;
    }
...
TEST(tape_sort, default_case) // num_test > max_elem_number
{
    ...
    // Assert
    std::vector<int32_t> output_data(std::move(TestClass::ReadTape(output_tape_name)));
...
TEST(config_reader, fields_search)
{
    ...
    uint32_t expected{ config_json.at("tape_delays").at("item_write").get<uint32_t>() };
